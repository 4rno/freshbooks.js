  ...
  for (var x = 0; x < nodes.length; x++) {
    if ("text" !== nodes[x].name()) {
      switch (nodes[x].name()) {
        ...
        case "links":
          (this.links.client_view = xml
            .get("//xmlns:client_view", this.freshbooks.ns)
            .text()),
            (this.links.view = xml
              .get("//xmlns:view", this.freshbooks.ns)
              .text()),
            (this.links.edit = xml
              .get("//xmlns:edit", this.freshbooks.ns)
              .text());
          break;
  ...
  this._setXML("invoice.create", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if (null !== err) {
        ...
        self.invoice_id = xml
          .get("//xmlns:invoice_id", self.freshbooks.ns)
          .text();
