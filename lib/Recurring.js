        ...
        if ("function" !== typeof this[key]) {
          switch (key) {
            ...
            case "autobill":
              ...
              expiration
                .node("month")
                .text(this.autobill.card.expiration.month);
  ...
  for (var x = 0; x < nodes.length; x++) {
    if ("text" !== nodes[x].name()) {
      switch (nodes[x].name()) {
        ...
        case "contacts":
          this.contact_id = xml
            .get("//xmlns:contact_id", this.freshbooks.ns)
            .text();
          break;
        ...
        case "autobill":
          if (xml.get("//xmlns:gateway_name", this.freshbooks.ns)) {
            ...
            (this.autobill.card = {}),
              ...
              (this.autobill.gateway_name = xml
                .get("//xmlns:gateway_name", this.freshbooks.ns)
                .text()),
              (this.autobill.card.number = xml
                .get("//xmlns:number", this.freshbooks.ns)
                .text()),
              (this.autobill.card.name = xml
                .get("//xmlns:name", this.freshbooks.ns)
                .text()),
              (this.autobill.card.expiration.month = xml
                .get("//xmlns:month", this.freshbooks.ns)
                .text()),
              (this.autobill.card.expiration.year = xml
                .get("//xmlns:year", this.freshbooks.ns)
                .text());
          }
          break;
  ...
  this._setXML("recurring.create", function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if (null !== err) {
        ...
        self.recurring_id = xml
          .get("//xmlns:recurring_id", self.freshbooks.ns)
          .text();
